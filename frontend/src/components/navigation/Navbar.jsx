...
export function Navbar() {
  ...
  return <header className="site-header"><Logo /><button className="menu-button" onClick={() => setOpen(!open)} aria-label="Toggle navigation" aria-expanded={open}>☰</button><nav className={open ? 'nav open' : 'nav'} aria-label="Primary navigation">{PUBLIC_NAVIGATION.map(item => <button key={item.label} className={item.label === 'Home' ? 'nav-link active' : 'nav-link'} onClick={() => go(item)}>{item.label}</button>)}<div className="nav-actions">{isAuthenticated ? <><button className="login-button" onClick={() => { window.location.href = '/learning'; }}>Learning</button><button className="login-button" onClick={() => { logout(); window.location.href = '/'; }}>Logout</button></> : <><button className="login-button" onClick={() => { window.location.href = '/login'; }}>Log in</button><button className="button small" onClick={() => { window.location.href = '/signup'; }}>Get started <Arrow /></button></>}</div></nav></header>;
}
